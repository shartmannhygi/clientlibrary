...
use Upg\Library\Request\Finish as FinishRequest;
use Upg\Library\Api\Finish as FinishApi;
...
class FinishTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSuccessfulApiCall()
    {
        ...
        $captureRequest->setOrderID($orderId)
            ...
            ->setAmount($this->getPartialAmount());
         ...
         * Do the finish call
         */
        $finishRequest = new FinishRequest($this->config);
        $finishRequest->setOrderID($orderId);

        $finishApi = new FinishApi($this->config, $finishRequest);

        $result = $finishApi->sendRequest();
