...
class RegisterUserPaymentInstrumentTest extends \PHPUnit_Framework_TestCase
{
    ...
    private function getPaymentInstrument()
    {
        if (is_null($this->paymentInstrument)) {
            ...
            $type = trim(strtolower($this->faker->creditCardType));

            switch ($type) {
                case 'visa':
                    $type = PaymentInstrumentJson::ISSUER_VISA;
                    break;
                case 'mastercard':
                    $type = PaymentInstrumentJson::ISSUER_MC;
                    break;
                case 'american express':
                case 'discover card':
                    $type = PaymentInstrumentJson::ISSUER_AMEX;
                    break;
            }
    ...
    public function testSuccessfulApiCall()
    {
        ...
        $result = $registerUserPaymentInstrument->sendRequest();
        ...
        $this->assertNotEmpty($result->getData('paymentInstrumentID'));
    }
}
