...
class RefundTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSuccessfulApiCall()
    {
         ...
         * Do the refund
         */
        $refundRequest = new RefundRequest($this->config);
        $refundRequest->setOrderID($orderId)
            ->setCaptureID($captureId)
            ->setRefundDescription("Test Refund for integration tests")
            ->setAmount($this->getAmount());

        $refundApi = new RefundApi($this->config, $refundRequest);
        $result = $refundApi->sendRequest();
